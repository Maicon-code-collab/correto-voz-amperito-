...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private animation() {
    ...
    if (sphereMaterial.userData.shader) {
      // Makes the sphere scale react to both input and output audio.
      const combinedVolume =
        (this.inputAnalyser.data[1] + this.outputAnalyser.data[1]) / 2;
      this.sphere.scale.setScalar(1 + (0.2 * combinedVolume) / 255);
      ...
      // Increased the multiplier for the y-component to make distortion more visible.
      sphereMaterial.userData.shader.uniforms.inputData.value.set(
        (1 * this.inputAnalyser.data[0]) / 255,
        (0.5 * this.inputAnalyser.data[1]) / 255,
        (10 * this.inputAnalyser.data[2]) / 255,
        0,
      );
      sphereMaterial.userData.shader.uniforms.outputData.value.set(
        (2 * this.outputAnalyser.data[0]) / 255,
        (0.5 * this.outputAnalyser.data[1]) / 255,
